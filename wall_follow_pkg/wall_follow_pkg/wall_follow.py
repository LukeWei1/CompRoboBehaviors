...
class WallFollowNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(namespace='', parameters=[
            ...
            ('target_distance', 0.5),  # desired distance from the wall (m)
            ('forward_vel', 0.1),    # constant forward speed (m/s)
            ('side', 'right'),       # which side to follow
            ('theta_deg', 45.0),     # angle between the two beams [deg]
            ('safety_distance', 0.4),  # if the front beam sees closer than this, override steering 
    ...
    def run_loop(self):
        ...
        if self.front_dist is not None and self.front_dist < self.safety_distance:
            ...
            msg.linear.x = max(0.0, self.forward_vel
                                * (self.front_dist / self.safety_distance)**2)
